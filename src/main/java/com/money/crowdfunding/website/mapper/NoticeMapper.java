...
public interface NoticeMapper {
    ...
    //getAll
    @Select("select * from xinwentongzhi where biaoti like #{title} limit 12")
    List<XinWenTongZhi> selectNoticeListByLike(@Param("title")String title);


    @Insert("insert into xinwentongzhi(biaoti,leibie,neirong,tianjiaren,shouyetupian,dianjilv,addtime)values(#{biaoti},#{leibie},#{neirong},#{tianjiaren},#{shouyetupian},#{dianjilv},#{addtime})")
    int insertOneNotice(XinWenTongZhi xinWenTongZhi);
}
